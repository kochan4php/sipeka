                                ...
                                <td class="d-flex justify-content-center vertical-align-middle custom-font">
                                    @if ($item->alumni)
                                        @if (is_null($item->alumni->foto))
                                            <img src="{{ Avatar::create($item->alumni->nama_lengkap) }}"
                                                alt="{{ $item->user->username }}" width="40" class="rounded-circle">
                                        @else
                                            <img src="{{ asset('storage/' . $item->alumni->foto) }}"
                                                alt="{{ $item->user->username }}" width="40">
                                        @endif
                                    @else
                                        @if (is_null($item->masyarakat->foto))
                                            <img src="{{ Avatar::create($item->masyarakat->nama_lengkap) }}"
                                                alt="{{ $item->user->username }}" width="40" class="rounded-circle">
                                        @else
                                            <img src="{{ asset('storage/' . $item->masyarakat->foto) }}"
                                                alt="{{ $item->user->username }}" width="40">
                                        @endif
                                    @endif
                                </td>
